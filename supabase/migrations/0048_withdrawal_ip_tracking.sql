-- Motosu — trace l'adresse IP d'origine de chaque demande de retrait, pour
-- que le staff puisse repérer des schémas suspects avant validation (ex :
-- plusieurs comptes différents demandant un retrait depuis la même IP).
-- L'IP est capturée côté Edge Function (seul endroit qui voit le vrai
-- en-tête réseau du client) et transmise en paramètre à la RPC — jamais
-- fournie par le client lui-même, qui pourrait mentir dessus.

alter table public.withdrawal_requests add column if not exists ip_address text;

-- Un nouveau paramètre en fin de liste change la signature : `create or
-- replace` créerait une surcharge de plus au lieu de remplacer l'existante.
-- On la supprime explicitement pour n'avoir qu'une seule version.
drop function if exists public.create_withdrawal_request(bigint, text, jsonb, public.earning_bucket);

create or replace function public.create_withdrawal_request(
  p_amount bigint, p_method text, p_destination jsonb,
...
begin
  ...
  values (v_user, p_amount, v_fee, v_net, p_method, p_destination || jsonb_build_object('bucket', p_bucket), 'PENDING', p_ip_address)
...
-- `create or replace` sur une fonction fraîchement recréée repart d'un
-- GRANT PUBLIC implicite — on réapplique le même durcissement que le reste
-- des fonctions métier (anon ne doit jamais pouvoir l'appeler directement).
revoke execute on function public.create_withdrawal_request(bigint, text, jsonb, public.earning_bucket, text) from public, anon;
grant execute on function public.create_withdrawal_request(bigint, text, jsonb, public.earning_bucket, text) to authenticated;
